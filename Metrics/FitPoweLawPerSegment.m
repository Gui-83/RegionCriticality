function [seg_alpha, seg_beta, seg_gamma_exp, seg_xminS, seg_xmaxS, seg_xminT, seg_xmaxT,seg_gamma_area, seg_gamma_shape, seg_chi] = FitPoweLawPerSegment(R, region, S_all, T_all, aval_starts, aval_stops, all_segments, state, ind, opt)
...
for seg = 1:n_seg
    ...
    lenST_seg = lenST_all(in_seg);
        ...
        [beta_seg, xmin_S, xmax_S, p_S] = swipeMLEDiscretePowerLawBounds(S_seg, opt.p_threshold, opt.n_swipe, 0.1, 3, false);
        [alpha_seg, xmin_T, xmax_T, p_T] = swipeMLEDiscretePowerLawBounds(T_seg, opt.p_threshold, opt.n_swipe, 1, 3, false);
        seg_xminS(seg) = xmin_S;
        seg_xmaxS(seg) = xmax_S;
        seg_xminT(seg) = xmin_T;
        seg_xmaxT(seg) = xmax_T;

        %display([xmin_S, xmax_S, xmin_T, xmax_T]);
